...
(async () => {
  ...
  const applyState = async () => page.evaluate(() => ({
    ...
    shortsCssInjected: (() => { const st = document.querySelector('style[id^="ytp-"]'); const all = document.querySelectorAll('style[id^="ytp-"]'); for (const el of all) { if ((el.textContent || "").includes("m19.45,3.88")) return true; } return false; })(),
        shortsShellState: (() => { const shelves = document.querySelectorAll("ytd-rich-shelf-renderer"); for (const sh of shelves) { const isShorts = sh.querySelector("path[d*='m19.45,3.88']") || /^shorts$/i.test(((sh.querySelector("#title") || {}).textContent || "")); if (isShorts) { const t = sh.closest("ytd-rich-section-renderer") || sh; return getComputedStyle(t).display + (t.getAttribute("data-zen-shorts-dead") ? "+dead" : ""); } } return shelves.length ? "no-shorts-shelf" : "none-on-page"; })(),
