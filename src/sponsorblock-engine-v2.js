  ...
  const SponsorBlockEngine = (() => {
    ...
    const RATE_LIMIT_CAP_MS = 30 * 60 * 1000;
    ...
    const Settings = (() => {
      ...
      const onlySkipIfVip = () => !!S.sbOnlySkipIfVip;

      const incognito = () => !!S.sbIncognito;

      const skipCountdown = () => {
        const enabled = !!S.sbSkipCountdown;
        const time = Math.max(0, Math.min(5, Number(S.sbSkipCountdownTime) || 0));
        return { enabled, time };
      };

      const betterBeAwareCats = () => {
        const raw = typeof S.sbBetterBeAware === "string" ? S.sbBetterBeAware : "";
        const set = new Set();
      ...
      const showInSeekbar = (categoryId) => {
        const v = S["sb_" + categoryId + "_sb"];
      ...
      return {
        ...
        onlySkipIfVip, incognito, skipCountdown, betterBeAwareCats, showInSeekbar,
      };
    })();
    ...
    const Cache = (() => {
      ...
      const MAX_WRITTEN_KEYS = 1000;
      const scannedVideos = new Set();

      const set = async (videoId, configKey, segments) => {
        const cacheKey = "sb:" + videoId + ":" + configKey;
        if (!writtenKeys.has(cacheKey)) {
          if (writtenKeys.size >= MAX_WRITTEN_KEYS) writtenKeys.clear();
      ...
      const invalidate = async (videoId) => {
        ...
        for (const key of Array.from(writtenKeys)) {
          if (key.startsWith(prefix)) cacheKeys.add(key);
        }

        if (!scannedVideos.has(videoId)) {
          scannedVideos.add(videoId);
          try {
    ...
    const API = (() => {
      ...
      const buildFetchPlans = async (videoId, usePrivacy, categories, actionTypes) => {
        ...
        if (usePrivacy) {
          ...
          prefix = prefix.slice(0, PRIVACY_HASH_LENGTH);
        }
        const plans = [];
      ...
      const normalizeSegments = (payload, videoId, fullHash) => {
        ...
        const survivors = normalized.filter((s) => s.segment[1] > s.segment[0]);

        for (let i = 0; i < survivors.length - 1; i++) {
          const cur = survivors[i], next = survivors[i + 1];
          if (next.segment[0] < segmentEndFor(cur)) next.segment[0] = segmentEndFor(cur);
        }
        return {

          segments: survivors.filter((s) => s.segment[1] > s.segment[0]),
      ...
      const fetchSegments = async (videoId, abortSignal) => {
        ...
        if (!plans.length) {
          ...
          Metrics.recordLookupTrace([{ plan: "privacy-unavailable", status: 0, count: 0 }]);
          return [];
        }
        const trace = [];
        let lastError = null;
      ...
      const postVote = async (params) => {
        ...
        if (Settings.incognito()) return { ok: false, status: 0, message: "Incognito mode" };
        ...
        query.set("UUID", String(params.UUID));
      ...
      const rememberOwnedSegments = (videoId, known) => {
        let remembered = 0;
        try {
          if (!State.userId) return 0;
          for (const sg of State.segments) {
            if (
              sg && sg.userID === State.userId &&
              !(known && typeof known.has === "function" && known.has(sg.UUID)) &&
              !/^(idx-|preview-)/.test(String(sg.UUID))
            ) {
              rememberSubmission(sg.UUID, videoId);
              remembered++;
            }
          }
        } catch (_) {}
    ...
    const Player = (() => {
      ...
      const clearCountdownOverlay = () => {
        if (State.countdownTaskId) { try { clearTimeout(State.countdownTaskId); } catch (_) {} State.countdownTaskId = 0; }
        const el = document.getElementById("yt-zen-sb-countdown");
        if (el) { try { el.remove(); } catch (_) {} }
      };

      const scheduleCountdownSkip = (seg, uuid, targetTime, savedSec) => {
        clearCountdownOverlay();
        const videoEl = ie.el();
        if (!videoEl) return;
        const cd = Settings.skipCountdown();
        const remaining = Math.max(1, Math.min(5, Math.round(cd.time)));
        let overlay = document.getElementById("yt-zen-sb-countdown");
        if (!overlay) {
          overlay = document.createElement("div");
          overlay.id = "yt-zen-sb-countdown";
          overlay.className = "yt-zen-sb-countdown";
          try { document.body.appendChild(overlay); } catch (_) {}
        }
        overlay.innerHTML = "";
        const text = document.createElement("span");
        text.textContent = "Skipping " + savedSec.toFixed(1) + "s in " + remaining + "…";
        const cancel = document.createElement("button");
        cancel.textContent = "Cancel";
        cancel.addEventListener("click", () => {
          clearCountdownOverlay();
          State.processedUUIDs.add(uuid);
          try { pe("SponsorBlock skip cancelled", 1400, "info"); } catch (_) {}
        });
        overlay.appendChild(text);
        overlay.appendChild(cancel);
        State.countdownTaskId = setTimeout(() => {
          State.countdownTaskId = 0;
      ...
      const jumpToHighlight = () => {
        ...
        const hl = State.segments.find((s) => s.category === "poi_highlight" && s.actionType === "poi");
        if (!hl) return false;
        try { videoEl.currentTime = Math.max(0, hl.segment[0] + 0.01); } catch (_) {}
        return true;
      };
      ...
      const handlePlaybackTick = () => {
        ...
        if (action === "skip") {
          ...
          if (Settings.betterBeAwareCats().has(seg.category) && !State.betterBeAwareWarned.has(seg.UUID)) {
            State.betterBeAwareWarned.add(seg.UUID);
            const meta = Categories.find(c => c.id === seg.category);
            try { pe("Heads up — " + (meta ? meta.label : seg.category) + " segment", 2600, "info"); } catch (_) {}
            return;
          }

          if (Settings.onlySkipIfVip() && seg.userID && State.vipByUserID.get(seg.userID) === false) {
            return;
          }
          ...
          if (State.undoneUUIDs.has(seg.UUID)) return;
          ...
          if (!shouldSkipGuard(targetTime)) {
            const cd = Settings.skipCountdown();
            if (cd.enabled && cd.time > 0 && !State.countdownTaskId) {
              scheduleCountdownSkip(seg, uuid, targetTime, savedSec);
            } else {
    ...
    const UI = (() => {
      ...
      const renderSeekbarMarks = () => {
        ...
        for (let idx = 0; idx < segments.length; idx++) {
          ...
          if (!Settings.showInSeekbar(seg.category)) continue;
      ...
      const removePreviewSegments = () => {
        const before = State.segments.length;
        State.segments = State.segments.filter((sg) => !(sg.UUID && /^preview-uuid-/.test(sg.UUID)));
      ...
      const addPreviewSegment = (start, end, category, description) => {
        const s = Number(start), e = Number(end);
        if (!Number.isFinite(s) || !Number.isFinite(e) || s < 0 || e <= s) return false;
        removePreviewSegments();
        State.segments.push({
          category: category || "sponsor",
          segment: [s, e],
          UUID: "preview-uuid-" + Date.now() + "-" + Math.floor(Math.random() * 1e6),
          actionType: "skip",
          votes: 1,
          locked: 0,
      ...
      const showFloatingHUD = (seg, savedSec) => {
        ...
        hud.querySelector("#hud-undo-btn").addEventListener("click", () => {
          ...
          try {
            ...
            State.undoneUUIDs.add(seg.UUID);
            if (State.undoneUUIDs.size > 4096) {
              const first = State.undoneUUIDs.values().next().value;
              if (first !== undefined) State.undoneUUIDs.delete(first);
            }
      ...
      const renderEditorUI = () => {
        ...
        panel.querySelector("#ed-preview").addEventListener("click", () => {
          ...
          if (addPreviewSegment(start, end, State.editor.category, State.editor.description)) {
            ...
            .then(result => {
              if (result.ok) {
                ...
                if (returned.length) {
                  ...
                  setTimeout(() => API.rememberOwnedSegments(videoId, known), 2500);
                }
                closeSubmissionEditor();
              } else {
      ...
      const closeSubmissionEditor = () => {
        ...
        try { removePreviewSegments(); } catch (_) {}
    ...
    const VIP_MAP_CAP = 256;
    const rememberVip = (uid, vip) => {
      if (!uid) return;
      if (State.vipByUserID.has(uid)) State.vipByUserID.delete(uid);
      State.vipByUserID.set(uid, !!vip);
      while (State.vipByUserID.size > VIP_MAP_CAP) {
        const oldest = State.vipByUserID.keys().next().value;
        if (oldest === undefined) break;
        State.vipByUserID.delete(oldest);
      }
    };

    const resolveVipForSegments = (segments) => {
      if (!Settings.onlySkipIfVip()) return;
      const ids = [];
      for (const s of segments) {
        if (s && s.userID && !State.vipByUserID.has(s.userID)) ids.push(s.userID);
      }
      for (const uid of ids) {
        API.getIsUserVIP(uid, null).then((vip) => {
          rememberVip(uid, vip);
        }).catch(() => { rememberVip(uid, false); });
      }
    };
    ...
    const applySegments = (videoId, generation, segments, hidden, meta = {}) => {
      ...
      State.activeSegmentIndex = -1;
      resolveVipForSegments(State.segments);
      if (hidden) {
    ...
    const destroy = () => {
      ...
      State.betterBeAwareWarned.clear();
      State.vipByUserID.clear();
      State.lastWakeLookupAt = 0;
    };
    ...
    return {
      ...
      jumpToHighlight: () => Player.jumpToHighlight(),

      _audit: {
        addPreviewSegment: UI.addPreviewSegment,
        removePreviewSegments: UI.removePreviewSegments,
        closeSubmissionEditor: UI.closeSubmissionEditor,
        rememberOwnedSegments: API.rememberOwnedSegments,
        rememberVip,
        markUndone: (uuid) => { State.undoneUUIDs.add(String(uuid)); },
        isUndone: (uuid) => State.undoneUUIDs.has(String(uuid)),
        counts: () => ({
          segments: State.segments.length,
          previews: State.segments.filter((sg) => sg.UUID && /^preview-uuid-/.test(sg.UUID)).length,
          undone: State.undoneUUIDs.size,
          vip: State.vipByUserID.size,
          warned: State.betterBeAwareWarned.size,
          wakeAt: State.lastWakeLookupAt,
        }),
      },
      getConfigFlags: () => ({
        onlySkipIfVip: Settings.onlySkipIfVip(),
        incognito: Settings.incognito(),
        skipCountdown: Settings.skipCountdown(),
        betterBeAwareCats: Array.from(Settings.betterBeAwareCats()),
        showInSeekbar: (c) => Settings.showInSeekbar(c),
      })
    };
  })();
