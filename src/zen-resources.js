  ...
  const ZenResources = (() => {
      ...
      emit(event, payload) {
        ...
        if (direct) {
          for (const callback of Array.from(direct.values())) {
            try { reportAsyncError(callback(payload, meta)); count += 1; } catch (_) {}
          }
        }
        if (wildcard) {
          for (const callback of Array.from(wildcard.values())) {
            try { reportAsyncError(callback(payload, Object.assign({}, meta))); count += 1; } catch (_) {}
          }
        }
        return count;
      }
    ...
    class StateStore {
          ...
          try {
            ...
            if (value !== undefined && value !== null) {
              if (!this._mutated) {
                ...
                this._data && typeof this._data === "object" && !Array.isArray(this._data) &&
                value && typeof value === "object" && !Array.isArray(value)
              ) {

                for (const k of Object.keys(value)) {
                  if (!Object.prototype.hasOwnProperty.call(this._data, k)) this._data[k] = value[k];
                }
              }
            }
          } catch (_) {}
      ...
      async flush() {
        ...
        this._flushes++;
        try {
          await this._storage.set(this._key, snapshot);
          this._dirty = false;
          this._flushRetries = 0;
          return true;
        } catch (_) {
          this._flushRetries = (this._flushRetries || 0) + 1;
          if (this._flushRetries < 3 && !this._timer) {
            this._timer = setTimeout(() => {
              this._timer = 0;
              reportAsyncError(this.flush());
            }, this._flushMs * this._flushRetries);
            if (this._timer && typeof this._timer.unref === "function") {
              try { this._timer.unref(); } catch (_) {}
            }
          }
    ...
    const SharedObserver = (() => {
      ...
      const MAX_PENDING_MUTATIONS = 5000;
      let nextId = 1;
      let observerKey = "";
      ...
      const cancelFlushTimer = () => {
        if (rafHandle) {
          if (typeof cancelAnimationFrame === "function") { try { cancelAnimationFrame(rafHandle); } catch (_) {} }
          rafHandle = 0;
        }
      ...
      const scheduleFlush = () => {
        if ((rafHandle || timeoutHandle) || !pendingBatch.length) return;
        const doc = getDocument();
        const flush = () => {
          rafHandle = 0;
          timeoutHandle = 0;
        ...
        if (typeof requestAnimationFrame === "function" && doc && !doc.hidden) {
          ...
          timeoutHandle = setTimeout(flush, 16);
        }
      };
      ...
      const ensureObserver = () => {
        ...
        if (observer) {
          ...
          try {
            ...
            if (queued && queued.length) {
              for (let i = 0; i < queued.length; i++) pendingBatch.push(queued[i]);
              scheduleFlush();
            }
          } catch (_) {}
        ...
        observer = new MutationObserver((mutations) => {
          ...
          if (pendingBatch.length > MAX_PENDING_MUTATIONS) {
            pendingBatch.splice(0, pendingBatch.length - MAX_PENDING_MUTATIONS);
            droppedMutations++;
          }
          scheduleFlush();
        });
      ...
      return {
        ...
        stats: () => ({
          ...
          droppedMutations,
          pendingCap: MAX_PENDING_MUTATIONS,
        }),
      };
    })();
